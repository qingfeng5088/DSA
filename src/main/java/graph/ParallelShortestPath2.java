 ...
 * 整个地图就被抽象成一个有向有权图。
 */
public class ParallelShortestPath2<E> {// 有向有权图的邻接表表示
    private final List<Vertex<E>> adj; // 邻接表

    public ParallelShortestPath2() {
    ...
    public void dijkstra(E s, E t) {
        ...
        CompletableFuture<PriorityQueue<Vertex<E>>> future = CompletableFuture.supplyAsync(() -> {
            System.out.println("------queueA---Start-------");
            doQueue(vt, queueA, queueB);
            System.out.println("------queueA----end------queueB=" + queueB.size());
            return queueB;
        }, es);

        while ((!queueA.isEmpty() || !queueB.isEmpty())) {
            future.thenCompose(queue -> CompletableFuture.supplyAsync(() -> {
                System.out.println("------queueB---Start-------");
                doQueue(vt, queue, queueA);
                System.out.println("------queueB---end-------queueA=" + queueA.size());
                return queueA;
            }));
            if (isOver) {
                System.out.println("------isOver---Start-------");
                break;
            }
        }
...
//        es = Executors.newCachedThreadPool();
    ...
    public static void main(String[] args) {
        ParallelShortestPath2<String> shortestPath = new ParallelShortestPath2<>();
    ...
    private static void mixedTravelDemo(ParallelShortestPath2<String> shortestPath) {
    ...
    private static void shortestPathDemo(ParallelShortestPath2<String> shortestPath) {
