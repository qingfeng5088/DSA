...
public class RBTree<T extends Comparable<T>> {
    ...
    //-----------------------------打印树状结构--------------------------------
    // 用于获得树的层数
    public int getTreeDepth(RBTNode<T> root) {
    ...
    private void writeArray(RBTNode<T> currNode, int rowIndex, int columnIndex, String[][] res, int treeDepth) {
        // 保证输入的树不为空
        if (currNode == null) return;
        if (columnIndex > res[0].length - 1) return;

        // 先将当前节点保存到二维数组中
        res[rowIndex][columnIndex] = String.valueOf(currNode.key) + String.format("%s", isRed(currNode) ? "R" : "B");
